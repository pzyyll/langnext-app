...
- ESLint and oxfmt (formatting; migrated from Prettier)
...
mise run format        # Write oxfmt and cargo fmt changes
mise run format:check  # Check oxfmt and cargo fmt formatting
...
- Spaces only (`tabWidth: 2`). Never mix tabs and spaces, including comments; oxfmt/`format:check` does not catch comment-internal tabs.
...
- Exclude it from ESLint and oxfmt edits.
