// ABOUTME: Google Cloud language-detection Wasm guest targeting translate-detect-world.
// ABOUTME: Uses the Translation endpoint and host-derived OAuth without exposing auth material.
#![no_std]
...
impl Guest for Component {
  fn detect(
    ...
    let (relative_path, body) = protocol::detect_request_body(&request.text, &project, &location)
    ...
    let parsed = protocol::parse_detect_response(status, &body).map_err(map_protocol_error)?;
    Ok(DetectResponse {
