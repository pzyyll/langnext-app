// ABOUTME: Anthropic llm-models-world Component: lists models through the host broker with
// ABOUTME: bounded after_id page traversal; ports GET /v1/models; guest sends only the version header.
#![no_std]
...
/// pages append `after_id=<cursor>` to the relative path.
fn fetch_page(after_id: Option<&str>) -> Result<protocol::ModelsPage, PluginError> {
    let relative_path = match after_id {
        Some(cursor) => alloc::format!("{}?after_id={}", protocol::MODELS_PATH, cursor),
    ...
    let response = broker_fetch(BrokerRequest {
        ...
        // The guest requests only its non-secret version header; the host injects x-api-key.
        headers: vec![(
            String::from(protocol::ANTHROPIC_VERSION_HEADER),
            String::from(protocol::ANTHROPIC_VERSION),
        )],
...
/// Any page failure returns no partial list.
fn fetch_all_models() -> Result<Vec<(String, Option<String>)>, PluginError> {
    ...
    for _page in 0..protocol::MAX_MODELS_PAGES {
        let page = fetch_page(after_id.as_deref())?;
