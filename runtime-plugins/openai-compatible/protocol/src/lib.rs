// ABOUTME: Shared no_std OpenAI Compatible protocol: chat.completions body construction,
// ABOUTME: models/chat/SSE parsing, status mapping, preference envelopes; ports the TS plugin.
#![no_std]
...
/// `model`, `messages`, `stream`, then optional `temperature` and `max_tokens`. Roles and
/// content are copied semantic values; the optional image base64 is embedded as a data URL.
...
/// Check one provider status for chat/models responses: 2xx passes, else a bounded error.
