// ABOUTME: DeepSeek llm-chat-world Component: chat.completions unary + SSE streaming through
// ABOUTME: the host broker; thinking comes ONLY from the host envelope. Bindings: generated.rs.
#![no_std]
...
fn image_base64s(request: &ChatRequest<'_>) -> Result<Vec<String>, PluginError> {
    if request.images.len() > protocol::MAX_CHAT_IMAGES {
        return Err(PluginError::InvalidInput(String::from(
            "deepseek chat supports at most one image",
        )));
    }
...
/// optional temperature/max_tokens from the preference envelope, and the host-envelope
/// thinking payload. The thinking policy is host-owned; the guest only renders it.
...
fn fetch_complete(request: &ChatRequest<'_>, preferences: &protocol::LlmPreferences) -> Result<ChatResponse, PluginError> {
    ...
    let response = broker_fetch(BrokerRequest {
        ...
        relative_path: String::from(protocol::CHAT_COMPLETIONS_PATH),
...
impl Guest for Component {
    fn chat(
        ...
        // The host selects the mode: complete for stream = false, SSE stream deltas for
        // stream = true. The guest never infers the mode from protocol details.
