// ABOUTME: OpenAI Responses llm-models-world Component: lists models through the host broker.
// ABOUTME: Ports the current provider's GET /models wire contract and bounded page parsing.
#![no_std]
...
/// Provider models endpoint requested through the host broker (provider-instance auth).
const MODELS_PATH: &str = protocol::MODELS_PATH;
...
fn fetch_models() -> Result<Vec<String>, PluginError> {
    ...
    // defaults to the JSON response mode.
            ...
            "over-limit" => {
                ...
                for index in 0..600u32 {
                    models.push(ModelDescriptor {
                        id: alloc::format!("over-limit-model-{index}"),
                        label: None,
                    });
                }
