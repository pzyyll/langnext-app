// ABOUTME: OpenAI Responses llm-chat-world Component: /responses unary + typed event-stream
// ABOUTME: deltas through the host broker under the host-selected envelope. Bindings: generated.rs.
#![no_std]
...
use langnext_openai_responses_protocol as protocol;

/// Provider Responses endpoint requested through the host broker (provider-instance auth).
const RESPONSES_PATH: &str = protocol::RESPONSES_PATH;
...
fn image_base64s(request: &ChatRequest<'_>) -> Result<Vec<String>, PluginError> {
    if request.images.len() > protocol::MAX_CHAT_IMAGES {
        return Err(PluginError::InvalidInput(String::from(
            "openai-responses chat supports at most one image",
        )));
    }
...
/// Build the provider /responses body: copied semantic messages, the host-selected stream
/// flag, optional temperature/max_output_tokens from the preference envelope, and the image
/// data URL. The Responses API takes one user input; the guest pairs the last user message
/// with the image exactly like the current TypeScript plugin.
fn build_body(request: &ChatRequest<'_>, preferences: &protocol::LlmPreferences) -> Result<Vec<u8>, PluginError> {
    ...
    let body = protocol::build_responses_body(
        &request.model,
        system_prompt,
        user_prompt,
...
/// Fetch the provider /responses endpoint for a non-stream request and parse the complete
/// message (output_text convenience field or joined output text blocks).
fn fetch_complete(request: &ChatRequest<'_>, preferences: &protocol::LlmPreferences) -> Result<ChatResponse, PluginError> {
    ...
    let response = broker_fetch(BrokerRequest {
        ...
        relative_path: String::from(RESPONSES_PATH),
...
/// the host stream response mode) and write ordered text deltas to the output writer.
/// `[DONE]` terminates the stream with one `complete` frame; lifecycle events are ignored;
/// a malformed delta event fails closed.
fn fetch_and_stream(
        ...
        match stream_receive(&reader).map_err(|_| PluginError::InvalidResponse(String::from("chat stream read failed")))? {
            Some(StreamFrame::NetworkBinary(bytes)) => {
                for event in decoder.feed(&bytes) {
                    // `[DONE]` terminates the stream: one terminal complete frame, then
                    // seal the writer.
                    if event.data == protocol::SSE_DONE {
                        done = true;
                        continue;
                    }
                    match protocol::parse_stream_event(&event.data, event.event.as_deref()) {
                        ...
                        Ok(protocol::StreamEventOutcome::Error(message)) => {
                            // A terminal failure event surfaces as an invalid response; the
                            // guest never forwards provider text into logs or deltas.
                            return Err(PluginError::InvalidResponse(message));
                        }
