...
- [ ] Lock resource placement in capability signatures: OCR and Speech Recognize requests carry a host-owned input `blob-handle`; Speech Synthesize returns a host-owned output `blob-handle` plus media metadata; LLM Chat carries message metadata and zero or more image `blob-handle` values and returns `variant { complete(llm-response), streaming(stream-handle) }`, where the guest writes ordered typed delta frames and the host/frontend reads them.
- [ ] Lock the broker body union: request body is `variant { empty, json(list<u8>), blob(blob-handle) }`; response body is `variant { json(list<u8>), blob(blob-handle), stream(stream-handle) }`. Endpoint/auth policy remain host-resolved, JSON bytes are bounded/validated UTF-8, and binary/stream bodies never cross as base64.
- [ ] Define the complete v1 `blob-handle` and `stream-handle` resource operation signatures now: bounded create/write/read/close/discard, ownership/direction checks, ordered typed stream send/receive, terminal state, cancellation, and metadata. Handles are opaque host table indices scoped to one principal/request and cannot be persisted or reused.
