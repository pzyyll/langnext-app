	...
	return (
		<div className="flex flex-col gap-8">
			<section className="flex flex-col gap-3">
				<p className="text-label-sm font-normal text-neutral uppercase">{t("home.kicker")}</p>
				<h1 className="text-headline-md font-bold text-on-surface">{t("home.title")}</h1>
				<p className="max-w-2xl text-body-md text-neutral">
					<Trans
						i18nKey="home.description"
						components={{
							code: <code className="border border-line bg-code px-1.5 py-0.5 font-mono text-code-inline text-on-surface" />,
						}}
					/>
				</p>
			</section>
			...
			<section className="shadow-frame border border-line bg-surface p-gutter">
				...
				>
					...
					<input
						...
						className="h-control-height flex-1 rounded-none border border-line bg-surface px-3 text-body-tight font-normal text-on-surface placeholder:text-neutral focus:outline-2 focus:-outline-offset-1 focus:outline-on-surface"
					/>
				...
				{greetMsg ? (
					<p className="mt-4 text-body-tight text-neutral">
						{t("home.lastMessage")} <span className="font-bold text-on-surface">{greetMsg}</span>
					</p>
				) : null}
			</section>
				...
				<Dialog.Portal>
					...
					<Dialog.Popup className="shadow-frame fixed top-1/2 left-1/2 -mt-8 flex w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 border border-line bg-surface p-gutter text-on-surface transition-[scale,opacity] duration-100 ease-out data-ending-style:scale-[0.98] data-ending-style:opacity-0 data-starting-style:scale-[0.98] data-starting-style:opacity-0">
						<div className="flex flex-col gap-1">
							<Dialog.Title className="text-title-dialog font-bold text-on-surface">{t("home.dialogTitle")}</Dialog.Title>
