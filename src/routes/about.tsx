...
const codeClassName = "border border-line bg-code px-1.5 py-0.5 font-mono text-code-inline text-ink";
	...
	return (
		<div className="flex flex-col gap-6">
			<section className="flex flex-col gap-2">
				<h1 className="text-headline-md font-bold text-ink">{t("about.title")}</h1>
			...
			<ul className="grid gap-3 sm:grid-cols-2">
				{stackKeys.map((key) => (
					<li key={key} className="shadow-frame border border-line bg-surface p-gutter">
						<div className="text-body-bold font-bold text-ink">{t(`about.stack.${key}.name`)}</div>
						<p className="mt-1 text-body-tight text-muted">{t(`about.stack.${key}.detail`)}</p>
					</li>
				))}
			</ul>
		</div>
