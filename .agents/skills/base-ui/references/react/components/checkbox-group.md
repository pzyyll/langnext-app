...
- **Form controls must have an accessible name**: It can be created using `<label>` elements, or the `Field` and `Fieldset` components. See [Labeling a checkbox group](./checkbox-group.md) and the [forms guide](../handbook/forms.md).

## Anatomy

Checkbox Group is composed together with [Checkbox](./checkbox.md). Import the components and place them together:
...
Use [Field](./field.md) and [Fieldset](./fieldset.md) for group labeling and form integration:
...
The group controls the parent checkbox's [indeterminate](./checkbox.md) state when some, but not all, child checkboxes are checked.
...
Re-export of [CheckboxGroup](./checkbox-group.md) props.
